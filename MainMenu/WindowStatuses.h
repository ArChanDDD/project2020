	...
	while (isSet) {
		...
		if (Mouse::isButtonPressed(Mouse::Left)) {
			if (setNum == 1 && !FullScreen) { window.setSize(FullScreenSize); WIDTH = FullScreenSize.x; HEIGHT = FullScreenSize.y; window.setPosition(ZeroPosition); FullScreen = true; } //FullScreen, работает, но хреново, лучше исправить.
				...
				if (setNum == 1 && FullScreen) { window.setSize(NotFullScreenSize); WIDTH = 500; HEIGHT = 500; window.setPosition(StartPosition); FullScreen = false;  }					      //¬ыход из FullScreen
...
void MainMenu(RenderWindow& window) {
	...
	bool isMenu = 1;												// провер€ем, что действи€ в данном меню не закончены	
	int menuNum = 0;												// Ќомер следующего меню
